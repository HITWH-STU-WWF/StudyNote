...
class DataClean():
    ...
    def updateBasic(self, file):
        ...
        pd_data.rename(index=str, columns={"学号": "stuID", "班号": "stuClassNumber", "姓名": "stuName", "性别": "sex", "民族": "nationality", "政治面貌": "politicalLandscape", "学历": "stuEducation", "身份证号": "idNumber", "公寓号": "apartmentNumber", "寝室号": "dormitoryNumber",
                                           "毕业高中": "graduatedHighSchool", "学生手机号": "stuMobileNumber", "家庭地址": "homeAddress", "父亲姓名": "fatherName", "父亲工作单位": "fatherWorkUnit", "父亲联系方式": "fatherMobileNumber", "母亲姓名": "motherName", "母亲工作单位": "motherWorkUnit", "母亲联系方式": "motherMobileNumber"}, inplace=True)
        cols = pd_data.columns.tolist()
        if set(['apartmentNumber', 'dormitoryNumber', 'fatherMobileNumber', 'fatherName', 'fatherWorkUnit', 'graduatedHighSchool', 'homeAddress', 'idNumber', 'motherMobileNumber', 'motherName', 'motherWorkUnit', 'nationality', 'politicalLandscape', 'sex', 'stuClassNumber', 'stuEducation', 'stuID', 'stuMobileNumber', 'stuName']) != set(cols):
        ...
        pd_data["ifPoor"] = 0
        pd_data["specialitiesid"] = pd_data["stuID"].apply(lambda x: str(x)[2:5])
        pd_data["collegeid"] = pd_data["stuID"].apply(lambda x: str(x)[2:4])
        pd_data["grade"] = pd_data["stuID"].apply(lambda x: str(x)[0:2])
